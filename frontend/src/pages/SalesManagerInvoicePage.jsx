...
export default function SalesManagerInvoicePage() {
  ...
  useEffect(() => {
    const token = sessionStorage.getItem('token');
    if (!token) { navigate('/login?next=/sales-manager/invoices'); return; }
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/sales-manager/invoices${qs}`), { headers: authHeaders() });
  ...
  useEffect(() => { if (authState === 'authorized') fetchInvoices(); }, [authState, fetchInvoices]);

  const handleDownloadPdf = async (orderId, invoiceNumber) => {
    setDownloadingId(orderId);
    try {
      const res = await fetch(apiUrl(`/api/orders/${orderId}/invoice/pdf`), { headers: authHeaders() });
  ...
  const handlePrint = () => {
    ...
    const rows = invoices.map((inv) => `
      <tr>
        <td>${inv.invoiceNumber}</td><td>${inv.customerName || '—'}</td>
        <td>${inv.customerEmail}</td><td>${formatDate(inv.createdAt)}</td>
        <td style="text-align:right">${formatCurrency(inv.total, inv.currency)}</td>
        <td>${inv.status}</td>
      </tr>`).join('');
    const win = window.open('', '_blank');
    win.document.write(`<!DOCTYPE html><html><head><title>Invoice Report — SCYLLA</title>
      <style>body{font-family:Georgia,serif;margin:40px;color:#111}h1{font-size:24px;font-weight:normal;margin-bottom:4px}
      .meta{font-size:12px;color:#666;margin-bottom:24px}table{width:100%;border-collapse:collapse;font-size:13px}
      ...
      td{padding:6px 8px;border-bottom:1px solid #ddd}tfoot td{border-top:2px solid #111;font-weight:bold}</style>
      </head><body>
      ...
      <table><thead><tr><th>Invoice #</th><th>Customer</th><th>Email</th><th>Date</th><th style="text-align:right">Total</th><th>Status</th></tr></thead>
      <tbody>${rows}</tbody>
      <tfoot><tr><td colspan="4">Total Revenue</td><td style="text-align:right">${formatCurrency(totalRevenue)}</td><td></td></tr></tfoot>
      </table></body></html>`);
    ...
    <>
      ...
      <div style={styles.page} ref={printRef}>
        ...
        <div style={styles.filterBar}>
          ...
          <button style={styles.filterBtn} onClick={fetchInvoices} disabled={loading}>{loading ? 'Loading…' : 'Apply Filter'}</button>
          {(startDate || endDate) && <button style={styles.clearBtn} onClick={() => { setStartDate(''); setEndDate(''); }}>Clear</button>}
        </div>

        {!loading && !error && (
          <div style={styles.summaryRow}>
            <div style={styles.summaryCard}><span style={styles.summaryLabel}>Total Orders</span><span style={styles.summaryValue}>{invoices.length}</span></div>
            <div style={styles.summaryCard}><span style={styles.summaryLabel}>Total Revenue</span><span style={styles.summaryValue}>{formatCurrency(totalRevenue)}</span></div>
          </div>
        )}
        ...
        {!error && (
          <div style={styles.tableWrap}>
            {loading ? <p style={styles.loadingText}>Loading invoices…</p>
              : invoices.length === 0 ? <p style={styles.emptyText}>No invoices found for the selected period.</p>
              : (
                <table style={styles.table}>
                  <thead>
                    <tr>{['Invoice #', 'Customer', 'Email', 'Date', 'Total', 'Status', 'PDF'].map((h) => <th key={h} style={styles.th}>{h}</th>)}</tr>
                  </thead>
                  <tbody>
                    {invoices.map((inv) => (
                      <tr key={inv.orderId} style={styles.tr}>
                        ...
                        <td style={styles.td}><span style={{ ...styles.statusBadge, ...statusColor(inv.status) }}>{inv.status}</span></td>
                        <td style={styles.td}>
                          <button style={styles.downloadBtn} disabled={downloadingId === inv.orderId} onClick={() => handleDownloadPdf(inv.orderId, inv.invoiceNumber)}>
...
const styles = {
  ...
  titleRow: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 'var(--space-3)' },
  ...
  printBtn: { fontFamily: 'var(--font-body)', fontSize: 'var(--text-sm)', backgroundColor: 'transparent', color: 'var(--color-charcoal)', border: '1px solid var(--color-charcoal)', borderRadius: 'var(--radius-md)', padding: 'var(--space-2) var(--space-5)', cursor: 'pointer' },
  ...
  th: { fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: 'var(--color-charcoal-light)', textTransform: 'uppercase', letterSpacing: '0.05em', padding: 'var(--space-3) var(--space-4)', borderBottom: '1px solid var(--color-border)', textAlign: 'left', whiteSpace: 'nowrap' },
  tr: { borderBottom: '1px solid var(--color-border)' },
  td: { fontFamily: 'var(--font-body)', fontSize: 'var(--text-sm)', color: 'var(--color-charcoal)', padding: 'var(--space-3) var(--space-4)', verticalAlign: 'middle' },
  invNum: { fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', color: 'var(--color-charcoal-light)' },
  statusBadge: { display: 'inline-block', padding: '2px 10px', borderRadius: '999px', fontSize: 'var(--text-xs)', fontFamily: 'var(--font-body)', textTransform: 'capitalize' },
  downloadBtn: { fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', backgroundColor: 'var(--color-black)', color: 'var(--color-white)', border: 'none', borderRadius: 'var(--radius-md)', padding: '4px 12px', cursor: 'pointer' },
